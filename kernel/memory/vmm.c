...
map_pages:
	...
	uint32_t conv_flags = convert_x86_32_vm_flags(flags);	
	if (args == 0) {
		for (int i = found; i < found + length; i+=4096) {
			memory_map(current_vmm->root_pd, alloc_phys_page(), (uint32_t*)i, conv_flags); // TODO: Change to flags
		}
	} else {
		for (int i = found; i < found + length; i+=4096) {
			memory_map(current_vmm->root_pd, (uint32_t*)((uint32_t)args+(i-found)), (uint32_t*)i, conv_flags); // TODO: Change to flags
		}
	}
